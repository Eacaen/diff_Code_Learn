		...
		for i in range(len(self.following)):
			...
			for i in range(len( img_urls )):
				...
				if img_urls[i] not in self.img_url_list:
					...
					if saveImg:
						self.saveImg(img_urls[i] , mdir = self.dir , filename = str(IMG_NAME) + '.'+ la)
						
						print IMG_NAME, '    ', la

					FILE_LOCK.acquire()	
					url_write = str(IMG_NAME) + '. ' + str( img_urls[i] ) + '\n'
...
if __name__ == '__main__': 
	...
	IMG_NAME =  get_img_url_num(img_url_list_txt) + 1
	...
	for i in xrange(_WORKER_THREAD_NUM) :
		thread = MyThread(zhihu , ( ques_code , img_patton ) )
