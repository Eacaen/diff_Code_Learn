...
def worker() :
    ...
    while not SHARE_Q.empty():
        ...
        patt = re.compile('start=(.*?)&filter=&type=')
        url_page = re.findall(patt , url)[0]

        my_page = get_page(url)
        find_title(my_page , int(url_page))  #获得当前页面的电影名
        time.sleep(1)
        SHARE_Q.task_done()
...
def find_title(my_page , url_num = 0) :
    """
    ...
    """
    temp_data = []
    page = range(url_num+1 , url_num + 30)
    ...
    for n in range(0 , len(temp_data) ):
        temp_data[n] = str(page[n]) + '. ' + temp_data[n]
